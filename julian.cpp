...
//
long JulianDate::YmdToJd( const int iYear, const int iMonth, const int iDay )
{
    long jul_day;
...
//
void JulianDate::JdToYmd( const long lJD, int *piYear, int *piMonth, int *piDay )
{
#ifndef JULDATE_USE_ALTERNATE_METHOD
...
// the NUL terminator;
//
void JulianDate::ToString( char *szBuffer ) const
{
    int i;
    long Temp;
...
int JulianDate::DayOfYear( void ) const
{
    int y, m, d;
    long soy;
...
//
time_t JulianDate::ToSysTime( void ) const
{
    struct tm tmRep;
    int y, m, d;
    time_t t;
